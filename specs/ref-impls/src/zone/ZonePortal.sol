...
contract ZonePortal is IZonePortal {
    ...
    /// @notice Fixed gas value for deposit processing.
    ...
    /// @notice Tempo gas rate used for deposit and withdrawal fees.
    ...
    /// @dev Fee = (FIXED_DEPOSIT_GAS + FIXED_BOUNCEBACK_GAS) * tempoGasRate.
    /// @return fee The deposit fee in zone token units.
    function calculateDepositFee() public view returns (uint128 fee) {
        fee = uint128(FIXED_DEPOSIT_GAS + FIXED_BOUNCEBACK_GAS) * tempoGasRate;
    }
        ...
        returns (bytes32 newCurrentDepositQueueHash)
    {
        ...
        // The user pays the up-front fee in addition to the principal; `amount` is delivered
        // on success or refunded on failure.
        uint128 fee = calculateDepositFee();
        if (amount > type(uint128).max - fee) revert DepositTooSmall();
        ...
        onlySequencer
    {
        ...
        if (_isDepositBounceBack(withdrawal)) {
            _processDepositBounceBack(_token, withdrawal.to, withdrawal.amount);
            return;
        }
