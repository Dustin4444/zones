...
| `zone_getZoneInfo` | Any authenticated | Returns `zoneId`, `zoneTokens`, `sequencer`, `chainId`, `accessMode`, and `gatewayMode` |
...
interface IZoneFactory {
    ...
    event ZoneCreated(
        ...
        address admin, address sequencer, address verifier,
...
Reads the sequencer address, token registry, enforcement modes, account membership, gateway membership, and encryption key from the portal on Tempo via `TempoState` storage reads.
