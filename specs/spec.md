...
2. The user derives an AES-256 key from the shared secret using HKDF-SHA256. The
   HKDF `info` binds the key to the expected Tempo deposit sender, so the client
   must derive the key using the address that will call `depositEncrypted`.
...
2. **AES-GCM decryption.** The zone derives an AES-256 key from the shared secret using HKDF-SHA256 (implemented in Solidity using the SHA256 precompile at `0x02`). The HKDF `info` is the canonical 104-byte packed encoding `[tempoPortal (20 bytes)][keyIndex (32 bytes, big-endian)][ephemeralPubkeyX (32 bytes)][sender (20 bytes)]`, where `sender` is the public sender recorded by the portal for that deposit. The depositor must use the same expected sender when encrypting. Consequently, replaying the encrypted payload from another account derives a different AES key and fails GCM authentication. Binding the sender in HKDF does not change the 64-byte plaintext or ciphertext, nor does it add calldata. The [AES-GCM Decrypt](#aes-gcm-decrypt) precompile decrypts the ciphertext and validates the GCM authentication tag. The plaintext is packed as `[address (20 bytes)][memo (32 bytes)][padding (12 bytes)]` totaling 64 bytes; the zone parses `(to, memo)` directly from it and uses those values for the mint.
