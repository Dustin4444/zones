...
The zone reads all of its configuration from Tempo: the sequencer address, the token registry and token-enablement commitment, the deposit queue hash, the Portal withdrawal queue head, and TIP-403 policy state. These reads use the finalized Tempo checkpoint.
