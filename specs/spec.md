...
3. The user encrypts `(to || memo || padding)` with AES-256-GCM using a fresh random 96-bit nonce, producing ciphertext and an authentication tag.
...
`senderWitness` is the 32-byte opening value for this commitment, not a signing or encryption key. The client must generate it with a cryptographically secure random-number generator, retain it for any later manual disclosure, and never use a counter or reuse it for another withdrawal. It is hidden from public L1 observers because it appears only in private Zone transaction data and state, but it is visible to the Zone sequencer. Choosing it before transaction construction lets the client compute both `senderTag` and any routed callback ciphertext without predicting the `fallbackNonce` assigned during execution. The outbox derives the tag from its immutable Tempo portal and actual `msg.sender`, records it as used before the token burn, and rejects duplicate user tags. Consequently multiple withdrawals from one transaction require different witnesses. Internal deposit bounce-backs remain exempt and retain the canonical `keccak256(address(0) || bytes32(0))` tag.
...
Here, `uint256_be` is the 32-byte big-endian encoding of the private scalar. Using `withdrawalHmacKey`, the sequencer derives the ECIES ephemeral scalar deterministically from the zone id, `revealTo`, `sender`, `senderWitness`, and the 8-byte big-endian `fallbackNonce`, retrying with a counter if the derived value is not a valid secp256k1 scalar. It derives the AES-GCM nonce from the same context plus the resulting ephemeral public key, making the same withdrawal byte-for-byte reproducible. This `encryptedSender` nonce is separate from the routed `DepositPayload.nonce`: `fallbackNonce` remains additional context for deterministic reveal encryption and refund routing, but is not part of `senderTag` or `routedDepositNonce`.
...
When `requestWithdrawal` executes, the outbox emits `senderTag`, marks it used, and stores the sender and witness. During finalization it recomputes the same tag from those committed inputs rather than storing a duplicate. The sequencer supplies only `encryptedSender`; this is trusted because a malicious sequencer could provide an incorrect ciphertext or omit it. The sender commitment remains deterministic and is covered by the same state transition checks as the rest of the withdrawal queue.
