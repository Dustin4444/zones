...
For an in-limit callback withdrawal, the portal first requires enough remaining transaction gas to cover the callback's `gasLimit` plus the fixed 1,750,000 gas callback-processing allowance used by the sequencer planner. An underfunded call reverts before changing the queue. The portal then dequeues before executing the withdrawal and independently requires `withdrawal.token` to be enabled. Failed callbacks roll back in an external self-call and become bounce-backs, so the dequeue remains committed and cannot block the FIFO. If `remainingQueue` is zero (last item in the slot), processing sets the slot to `EMPTY_SENTINEL` and advances `head`; otherwise it updates the slot to `remainingQueue`.
