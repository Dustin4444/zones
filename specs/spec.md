...
1. Requires `msg.sender`, `to`, and `bouncebackRecipient` to be allowed accounts. The gateway mapping is separate and never satisfies this check.
...
interface IZonePortal {
    ...
    // Deposits
    /// @dev Caller, `to`, and `bouncebackRecipient` must be allowed accounts.
    function deposit(
