...
For the bootstrap transition from zero to the canonical zone genesis block, `tempoBlockNumber` and `recentTempoBlockNumber` are both zero and the portal skips the EIP-2935 lookup. The genesis proof must show that `TempoState.tempoBlockHash` and `tempoBlockNumber` remain zero. Bootstrap carries empty deposit and withdrawal outputs, uses settlement withdrawal batch index zero, and initializes only the canonical zone block hash and height. It does not increment `withdrawalBatchIndex` or enqueue a withdrawal root. The first ordinary batch therefore extends the genesis hash and finalizes withdrawal batch index one.

After bootstrap, if `recentTempoBlockNumber` is `0`, the portal looks up `tempoBlockNumber` directly from EIP-2935. The proof must show that the zone's `tempoBlockHash` matches this hash. On the first Tempo import, the proof must additionally show that the portal's `admin` slot is non-zero in that block's state, proving that the imported block is not from before portal creation.
