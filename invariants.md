...
| `TEMPO-ZONE-PORTAL-PAIRING` | A `ZoneFactory` registry entry maps one zone ID to exactly one portal | 🟡 | Deposits, withdrawals, callbacks, and config reads can target different trust domains |
...
| `TEMPO-ZONE-CALLBACK-ATOMICITY` | Callback withdrawals transfer tokens and invoke the receiver inside one catchable external portal self-call | 🟡 | A failed callback can leak tokens instead of bouncing the withdrawal back |
