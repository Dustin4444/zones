...
| `TEMPO-ZONE-FIRST-TEMPO-ANCHOR` | When `TempoState.tempoBlockHash` is zero, the first imported Tempo block proves the portal's required admin is non-zero at that block | 🔴 | The zone may anchor to Tempo state from before its portal existed |
