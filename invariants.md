...
| `TEMPO-ZONE-WITHDRAWAL-CALLBACK-BOUNDS` | `gasLimit <= MAX_WITHDRAWAL_GAS_LIMIT`, callback data is bounded, and `processWithdrawals` requires the committed `gasLimit` plus the fixed callback-processing allowance before dequeueing; over-limit legacy withdrawals bounce back after dequeue | 🟡 | A sequencer can undergas a callback, force it to fail, and permanently bounce a withdrawal that would succeed with its committed stipend |
