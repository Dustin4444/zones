...
| `TEMPO-ZONE-SENDER-TAG-BINDING` | User `senderTag == keccak256(abi.encode(SENDER_TAG_DOMAIN, sourcePortal, sender, senderWitness))`; the outbox uses its actual caller and immutable portal, and rejects duplicate user tags before burn | 🟡 | Authenticated withdrawals can reveal, link, be forged, or collide as application nullifiers |
| `TEMPO-ZONE-ROUTER-NULLIFIER` | The router derives `routerWithdrawalId == keccak256(abi.encode(router, sourcePortal, senderTag))` only from authenticated messenger arguments and consumes its full 32 bytes once | 🟡 | A source withdrawal can be replayed or an attacker can consume another withdrawal's authorization |
| `TEMPO-ZONE-ROUTED-DEPOSIT-NONCE-BINDING` | The client encrypts and the router overwrites the GCM nonce with `bytes12(routerWithdrawalId)`; direct deposits retain random nonces | 🟡 | A copied encrypted payload can authenticate when submitted by another withdrawal through the shared router |
