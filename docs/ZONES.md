...
| `--l1.genesis-block-number` | `tempoAnchorBlock` from zone.json | Tempo block immediately before Portal creation; required until the Zone has imported its first checkpoint |
| `--zone.id` | 0 | Zone ID from ZoneFactory. At startup it must match `ZonePortal.zoneId()`; `0` does not bypass validation. The zone's chain ID is derived from the parent L1 chain ID and zone ID. |
