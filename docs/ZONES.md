...
| ZoneFeeManager | `0xfeec…0000` | **Replaced** — no user/validator preferences or FeeAMM routing. The explicit transaction token (or portal creation-time default) is escrowed directly, accrued by beneficiary/token, and claimable for that beneficiary by anyone. |
