...
| Proving system witness shape | Open | The prover now normalizes the current Zone state witness through an Alloy `ExecutionWitness` adapter before constructing the witness DB and state-root calculator, but the external witness payload and local generator still use custom Zone read containers. |
...
1. [ ] Replace the custom witness shape with an upstream-style execution witness adapter.
   Progress: `ZoneExecutionWitness` now adapts the current Zone state payload into Alloy's `ExecutionWitness` shape (`state`, `codes`, `keys`, `headers`), and production stateless preparation routes the witness DB and state-root calculator through that adapter. The remaining gap is that the serialized batch witness and local node generator still start from `ZoneAccountRead`, `ZoneStorageRead`, `proof_node_hashes`, and local proof conversion. Next step: make local generation emit the upstream execution witness directly, then keep custom formats only for Tempo L1 reads and Zone public inputs.
...
- Finish replacing hand-selected Zone proof collection by having local witness generation emit upstream-style execution witnesses directly.
