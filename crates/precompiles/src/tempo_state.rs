...
mod tests {
    ...
    use crate::test_utils::{
        MockL1Reader, TestCtx, call_precompile, call_precompile_with_gas, test_context,
    ...
    fn initialize(ctx: &mut TestCtx, header: &[u8]) -> eyre::Result<()> {
    ...
    fn initialize_sets_checkpoint() -> eyre::Result<()> {
        ...
        let precompile = TempoState::create(MockL1Reader::returning(B256::ZERO), &ctx.cfg.clone());
    ...
    fn delegate_call_reverts() -> eyre::Result<()> {
        ...
        let precompile = TempoState::create(MockL1Reader::returning(B256::ZERO), &ctx.cfg.clone());
        let output = call_precompile_with_gas(
            &mut ctx,
            &precompile,
            Address::ZERO,
