...
use genesis_artifacts::{TEMPO_STATE_ADDRESS, ZONE_INBOX_ADDRESS};
...
fn setup_zone_evm_with_contracts() -> genesis_artifacts::TestEvm {
    let mut header_rlp = Vec::new();
    tempo_primitives::TempoHeader::default().encode(&mut header_rlp);
    genesis_artifacts::setup_zone_evm_with_contracts_for_portal(
        zone_chain_id(1),
        Address::repeat_byte(0xbb),
        header_rlp,
    )
    .expect("deploy zone contracts from Foundry artifacts")
}
