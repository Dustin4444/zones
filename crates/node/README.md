...
5. Reth fast-path inserts the locally executed payload into its engine tree,
   then the engine **confirms** the L1 block in the deposit queue (removing
   it). On failure the block stays for retry.
