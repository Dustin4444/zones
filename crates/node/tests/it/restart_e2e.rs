...
async fn test_sequencer_restart_with_pending_withdrawal_queue() -> eyre::Result<()> {
    ...
    // Force even a simple withdrawal above the combined planner budget. The standalone
    // processor deliberately submits oversized singletons, which leaves a recoverable pending
    // portal slot after its first task is stopped below.
