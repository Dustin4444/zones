...
use crate::model::adapter::{ImportedProjectionError, PortalCreationIdentityError};
...
pub(crate) enum BootstrapError {
    ...
    #[error(transparent)]
    CreationProjection(#[from] ImportedProjectionError),
    #[error(transparent)]
    CreationIdentity(#[from] PortalCreationIdentityError),
    #[error("authenticated creation Portal mismatch: configured {expected}, event {actual}")]
    PortalMismatch { expected: Address, actual: Address },
    #[error("authenticated creation Zone ID mismatch: configured {expected}, event {actual}")]
    ZoneIdMismatch { expected: u32, actual: u32 },
    #[error(
        "authenticated creation initial token mismatch: Zone genesis uses {expected}, event uses {actual}"
    )]
    ...
    #[error(
        "durable L1 bootstrap cursor {cursor:?} is not on configured creation-to-anchor history"
    )]
    CursorNotOnBootstrapPath { cursor: BlockNumHash },
}
