...
use super::{acquisition, events, observe_l1, portal};
use crate::observe::{
...
fn direct_portal_call_requires_one_top_level_target_for_legacy_and_aa() {
    ...
    assert_eq!(
        portal::sole_portal_calldata(&direct, PORTAL, B256::ZERO).unwrap(),
        calldata.as_ref()
    );
    assert!(
        decode_portal_call(
    ...
    assert!(matches!(
        portal::sole_portal_calldata(&wrong_target, PORTAL, B256::ZERO),
    ...
    assert!(matches!(
        portal::sole_portal_calldata(&multi, PORTAL, B256::ZERO),
        Err(ObservationError::PortalCall(
            PortalCallError::UnsupportedNestedPortalCall { .. }
        ))
    ));
    ...
    assert_eq!(
        portal::sole_portal_calldata(&one_aa, PORTAL, B256::ZERO).unwrap(),
        calldata.as_ref()
    );
}
