//! Concrete archive bootstrap for the compact checker store.
...
use crate::{
    CheckerConfig,
    compact::{BuildConfig, project_expected_effect, publish_genesis_checkpoint},
    compact_observe::adapt_imported,
...
/// Build and atomically publish a compact checkpoint at local Zone genesis.
///
/// Every Tempo block is authenticated by exact hash and replayed only through
/// the compact kernel. No legacy projection or transition participates.
pub async fn build_compact_checkpoint<P>(
    ...
    P: BlockNumReader + StateProviderFactory + ?Sized,
{
    ...
    let [
        ...
        eyre::bail!("creation block must contain exactly one compact Create operation");
    };
...
async fn replay_one(
    ...
    if projection.effects != expected_effects {
        eyre::bail!("authenticated imported effects differ from compact candidate");
    }
