//! Milestone-5 compact builder and runtime primitives.
//!
//! This path is deliberately internal: the legacy ExEx remains the public
//! production runtime until cutover.  Observation adapters are the only
//! intended producers of [`AuthenticatedBlock`].

#![allow(dead_code)] // Internal shadow path until the Milestone-6 cutover.
...
pub(crate) fn compare_authenticated(
    ...
    if let Some((category, code)) = mismatch {
        return Err(typed_failure(
            ...
            "authenticated output differs from compact candidate",
        ));
    }
    Ok(())
}
...
fn validate_creation_coordinate(
    ...
    // Height zero is retained as the legacy/pre-creation-checkpoint sentinel.
...
pub(crate) struct CompactRuntime<N> {
    state: RuntimeState,
    current: Option<N>,
    fifo: VecDeque<N>,
    capacity: usize,
...
/// authenticated pipeline and compact kernel used by the live path. There is
...
/// Atomically publish a compact identity-bound genesis checkpoint assembled by
...
mod tests {
    ...
    fn bounded_fifo_keeps_one_current_and_order() {
        let mut runtime = CompactRuntime::new(2, RetryBudget::new(1, Duration::ZERO));
