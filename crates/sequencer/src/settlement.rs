...
pub struct BatchSubmitter {
    ...
    /// P2P signatures used once the portal activates quorum settlement.
...
impl BatchSubmitter {
    ...
    /// Attach the P2P attestation store used for quorum-certified submissions.
    ...
    pub async fn submit_batch(
        ...
        let set_version = self.portal.sequencerSetVersion().call().await?;
        let certificate = if set_version == 0 {
            None
        } else {
            let threshold = self.portal.sequencerThreshold().call().await? as usize;
            if threshold == 0 {
                return Err(eyre::eyre!(
                    "portal signer-set version {set_version} has zero threshold"
                ));
            }
            let store = self.attestation_store.as_ref().ok_or_else(|| {
                eyre::eyre!(
                    "portal signer-set version {set_version} requires P2P attestations, but no attestation store is configured"
                )
            })?;
            info!(
                zone_height,
                threshold, set_version, "Waiting for settlement quorum"
            );
            Some(store.wait_for_settlement(zone_height, threshold).await)
        };

        let anchor_mode = if let Some(certificate) = certificate.as_ref() {
            match self
                .validate_certificate(batch, zone_height, set_version, certificate)
                .await
            {
