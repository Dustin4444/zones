    ...
    ($($rpc_attr:tt)*) => {
        alloy_sol_types::sol! {
            ...
            contract ZonePortal {
                ...
                function setTempoGasRate(uint128 rate) external;
...
impl core::fmt::Display for ZonePortal::ZonePortalErrors {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::DepositTooSmall(_) => f.write_str("DepositTooSmall"),
...
impl Withdrawal {
    ...
    pub fn from_requested_event(
        ...
        let sender_tag_tx_hash = if event.sender == Address::ZERO
            && event.fallbackRecipient == Address::ZERO
            && event.gasLimit == 0
            && event.data.is_empty()
        {
            B256::ZERO
        ...
        Self {
            token: event.token,
            senderTag: Self::sender_tag(event.sender, sender_tag_tx_hash),
            to: event.to,
            amount: event.amount,
            fee: event.fee,
            memo: event.memo,
