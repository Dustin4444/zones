...
impl L1Subscriber {
    ...
    pub async fn run(mut self) -> eyre::Result<()> {
        ...
        loop {
            ...
            if let Some((tip_header, tip_events, tip_policy_events, tip_invalidated_accounts)) =
            ...
            unconfirmed_tip = Some((sealed, events, policy_events, invalidated_accounts));
        }
    ...
    fn extract_events(
        ...
        let mut invalidated_accounts = HashSet::new();
        ...
        for receipt in receipts {
            for log in receipt.logs() {
                ...
                if addr == portal_address {
                    ...
                    // Any registry log may accompany a storage mutation. Invalidate before
                    // decoding so unknown events introduced by future Tempo versions stay safe.
                ...
                {
                    // Transfer policy IDs remain in TIP-20 storage until Tempo migrates them into
                    // TIP-403, so invalidate the emitting token without knowing its slot layout.
                    invalidated_accounts.insert(addr);
        ...
        (portal_events, policy_events, invalidated_accounts)
    }
